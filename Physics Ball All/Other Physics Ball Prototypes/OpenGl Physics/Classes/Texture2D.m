...
color:(float[])color
{
	...
	CGSize dimensions = [string sizeWithFont:[UIFont fontWithName:name size:size]];
	...
	UIGraphicsPushContext(context);
		[string drawInRect:CGRectMake(0, 0, dimensions.width, dimensions.height) withFont:font lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentLeft];
...
- (id) initWithCircle:(float)radius
				color:(float[])color{
	NSUInteger				width,
	height,
	i;
	...
	CGSize dimensions = CGSizeMake(radius*2, radius*2);
	...
	CGContextFillEllipseInRect(context, CGRectMake(0, 0, dimensions.width, dimensions.height));
	//[string drawInRect:CGRectMake(0, 0, dimensions.width, dimensions.height) withFont:font lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentLeft];
	UIGraphicsPopContext();
...
- (id) initWithLine:(CGPoint)from to:(CGPoint)to thickness:(float)thickness
			  color:(float[])color{
	NSUInteger				width,
	height,
	i;
	...
	CGSize dimensions = CGSizeMake(abs(to.x-from.x), abs(to.y-from.y));
	...
	UIGraphicsPushContext(context);
		CGContextMoveToPoint(context, from.x, from.x);
